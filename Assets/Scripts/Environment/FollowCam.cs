...
public class FollowCam : MonoBehaviour {
	...
	void Update () 
    {
        ...
        if (transform.position.x - mFollowTarget.transform.position.x > mHorizontalDistanceFromCenter)
        { // If the target has moved to the left
            newX = mFollowTarget.transform.position.x + mHorizontalDistanceFromCenter;
	    }
        else if (mFollowTarget.transform.position.x - transform.position.x > mHorizontalDistanceFromCenter)
