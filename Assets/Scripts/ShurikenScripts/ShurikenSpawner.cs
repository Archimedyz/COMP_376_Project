...
public class ShurikenSpawner : MonoBehaviour
{
	...
	private void SetLayer (GameObject enemy)
	{
		if (shurikens [1].position.y <= -6.7f) {
			if (enemy.gameObject.name.Substring (0, 4) == "Hobo") {
				enemy.GetComponent<Hobo> ().SetFloorIndex (1);
			} else if (enemy.gameObject.name.Substring (0, 4) == "Nean") {
				enemy.GetComponent<Neanderthal> ().SetFloorIndex (1);
			}
		} else {
			if (enemy.gameObject.name.Substring (0, 4) == "Hobo") {
				enemy.GetComponent<Hobo> ().SetFloorIndex (0);
			} else if (enemy.gameObject.name.Substring (0, 4) == "Nean") {
				enemy.GetComponent<Neanderthal> ().SetFloorIndex (0);
			}
		}
	}
